...
%         nVar=0; 
    ...
    for K=K_set
        ...
        clear thetaLocal_set  
        %load local data
        for k=1:K
            path_name=['~Loacation\BayesianProbit_SyntheticData\Synthetic Data\Local Sampling\K=',num2str(K),'\SyntheticData_LocalSample_K',num2str(K),'_k=',num2str(k),'.mat'];
            load(path_name)
            thetaLocal_set(:,:,k)=theta(:,1:1e4);
            clear theta  
        end
...
%             sqrt_Qk_inv= u*diag(sqrt(inv_s))*v';
       ...
       clear D_gauss_recv  D_ana_recv 
        for k=1:K
            ...
            Err_cov_SingleWorker((shuffle_time-1)*interval+split_index,k_index,k)= sum(sum(abs(theta_single*theta_single'/Ns-Cov_global)./ abs(Cov_global)))/d/d;
        end

       Err_cov_GaussOMA((shuffle_time-1)*interval+split_index,k_index) = sum(sum(abs(est_theta_gauss*est_theta_gauss'/Ns-Cov_global)./abs(Cov_global)))/d/d;
       Err_cov_ana((shuffle_time-1)*interval+split_index,k_index) = sum(sum(abs(est_theta_ana*est_theta_ana'/Ns-Cov_global)./abs(Cov_global)))/d/d;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%         
        ...
        Err_cov_GD_OMA((shuffle_time-1)*interval+split_index,k_index)=Err_cov_GD(t);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
            ...
            Err_cov_wgcmcN((shuffle_time-1)*interval+split_index,k_index) = sum(sum(abs(est_theta_gcmcN*est_theta_gcmcN'/Ns_NOMA-Cov_global)./abs(Cov_global)))/d/d;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
        ...
        %  learning rate
        etaD=2*1e-6/K;        % for nb=500 1e-9; for GD 1e-7
        ...
        Err_cov_GD_NOMA((shuffle_time-1)*interval+split_index,k_index)=Err_cov_GD(t);
    ...
    figure(1)
    err_single=reshape(mean(Err_cov_SingleWorker,1),length(K_set),max(K_set)); 
    err_single(find(err_single==0))=10^10;
    semilogy(K_set, min(err_single'),'-.','LineWidth',2,'Color',[0.4940    0.1840    0.5560])
    hold on 
    semilogy(K_set,mean(Err_cov_GaussOMA,1),':','LineWidth',2,'Color',[0.9290    0.6940    0.1250])
    hold on 
    semilogy(K_set,mean(Err_cov_ana,1),'--d','LineWidth',2,'Color',[ 0.8500    0.3250    0.0980]);
    hold on 
    semilogy(K_set,mean(Err_cov_wgcmcN,1),'--*','LineWidth',2,'Color',[ 0    0.4470    0.7410]);
    hold on
    semilogy(K_set,mean(Err_cov_GD_OMA,1),'-d','LineWidth',2,'Color',[ 0.8500    0.3250    0.0980]);
    hold on 
    semilogy(K_set,mean(Err_cov_GD_NOMA,1),'-*','LineWidth',2,'Color',[ 0    0.4470    0.7410]);   
    hold off
